...
export default function Aquarium3D() {
  ...
  useEffect(() => {
    ...
    const restoreLatestLayout = async () => {
      try {
        const [tanks, catalog] = await Promise.all([
          getTanks(),
          getAquariumCatalog(),
        ]);
        ...
        const sameSizeTanks = tanks.filter((tank) =>
          isSameSize(tank.size, size),
        );
  ...
  return (
    <div className="aquarium3d-page">
      ...
      <div className="control-buttons">
        ...
        >
          {savingLayout
            ? t("AQUARIUM3D.SAVING_SHORT")
            : t("AQUARIUM3D.SAVE_SHORT")}
        </button>
      </div>
      ...
      {saveDialogOpen && (
        <div
          ...
          onClick={() => !savingLayout && setSaveDialogOpen(false)}
        >
          <div
          ...
          >
            ...
            <label
              className="save-dialog-label"
              htmlFor="tankImageUrlInputDialog"
            >
              Preview image URL
            </label>
            <input
            ...
            <label
              className="save-dialog-upload-btn"
              htmlFor="tankImageFileInputDialog"
            >
              Choose image file
            </label>
            <input
