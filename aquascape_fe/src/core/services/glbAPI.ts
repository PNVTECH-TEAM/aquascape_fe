...
export type GenerateGlbRequest = TripoGenerationSettings
...
tripoHttp.defaults.headers.common["bypass-tunnel-reminder"] = "true";
...
export const generateTripoAsset = async (
  ...
  const formData = createGenerationFormData(
    request,
    request.modelSaveFormat ?? "glb",
  );
  logTripoRequest("generate", "/generate", formData);

  const response = await tripoHttp.post("/generate", formData, {
    timeout: GENERATION_TIMEOUT_MS,
    headers: {
      "Content-Type": "multipart/form-data",
    },
...
export const downloadTripoAsset = async (
  ...
  const requestUrl =
    /^https?:\/\//i.test(downloadUrlOrJobId) ||
    downloadUrlOrJobId.startsWith("/")
      ? resolveTripoRequestUrl(downloadUrlOrJobId)
      : buildTripoDownloadUrl(downloadUrlOrJobId, options);
